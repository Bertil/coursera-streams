...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val nn = for {
      neigbour <- b.legalNeighbors
    } yield (neigbour._1, neigbour._2 :: history)
    nn.to(LazyList)
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filterNot(n => explored contains(n._1))
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(neighborsWithHistory(startBlock, Nil), Set(startBlock) )
